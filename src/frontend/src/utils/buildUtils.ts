...
/**
 * Polls build events from the server using NDJSON over HTTP long polling.
 * Cleans up polling when done or on error.
 */
async function pollBuildEvents(
  ...
  callbacks: {
    ...
    onBuildComplete?: (allNodesValid: boolean) => void;
    onBuildError?: (title: string, list: string[], idList?: VertexLayerElementType[]) => void;
    onGetOrderSuccess?: () => void;
    onValidateNodes?: (nodes: string[]) => void;
  },
  ...
  while (!isDone) {
    ...
    try {
      ...
      callbacks.onBuildError?.(
        "Connection Error",
        ["Failed to fetch build events. Please check your connection."],
      );
      break;
    }
    ...
    if (!response.ok) {
      ...
      callbacks.onBuildError?.(
        "Server Error",
        [errorData.detail || "Langflow was not able to connect to the server. Please make sure your connection is working properly."],
      );
      break;
    }
    ...
    for (const eventStr of eventLines) {
      try {
        ...
        callbacks.onBuildError?.(
          "Parse Error",
          ["Failed to parse build event."],
        );
        // Optionally continue or break here
      }
    }

    await delay(BUILD_POLLING_INTERVAL);
  }
}
